...
async function callGemini(images: string[], prompt: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    const res = await fetch(url, {
      ...
      // Keep output tokens moderate — we just need extracted text not full JSON
  ...
  try {
    ...
    if (action === "fill_remaining") {
      ...
      const raw = await generateText([
        { role: "system", content: `Generate exactly ${n} NEW MCQ questions about: "${topic || "same exam subject"}". No repeats. Return ONLY JSON array. Math in $...$. Options A. B. C. D. Always explain. Format: [{"id":1,"question":"...","options":["A. ...","B. ...","C. ...","D. ..."],"answer":"A","explanation":"..."}]` },
        { role: "user", content: `Generate exactly ${n} fresh questions. Return only the JSON array.` },
      ], 3000, isPremium);
      const rawQs = parseAIJson(raw);
      if (!rawQs?.length) return NextResponse.json({ error: "Could not generate additional questions." }, { status: 422 });
    ...
    if (images && images.length > 0) {
      // Step 1: Extract text from all pages (ONE Gemini call per batch, fallback to Groq)
...
console.log("[generate] combined snippet:", combined.slice(0, 300));
      ...
      if (!rawQs?.length) {
        ...
        const retryRaw = await generateText([
          { role: "system", content: `Return ONLY a JSON array. Format: [{"id":1,"question":"...","options":["A. ...","B. ...","C. ...","D. ..."],"answer":"A","explanation":"..."}]` },
          { role: "user", content: `Convert to JSON:\n\n${combined.slice(0, 4000)}` },
      ...
      return NextResponse.json({
        ...
        notice: needsChoice ? `Only ${totalFound} questions found. Try setting count to ${totalFound}.` : null,
      });
      ...
      const rawQs = parseAIJson(raw);
      if (!rawQs?.length) return NextResponse.json({ error: "Could not generate questions. Please try again." }, { status: 422 });
      ...
      return NextResponse.json({
        questions: normalized.slice(0, count),
        totalFound, requested: count, needsChoice,
        notice: needsChoice ? `Only ${totalFound} questions generated. Try setting count to ${totalFound}.` : null,
      });
    }
