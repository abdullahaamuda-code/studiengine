...
function parseAIJson(raw: string): any {
  // Bail early on obvious non-JSON responses
  ...
  const clean = trimmed.replace(/```json\s*/gi, "").replace(/```\s*/gi, "").trim();
...
function friendlyError(msg: string): string {
  if (msg.includes("too large") || msg.includes("TPM") || msg.includes("TPD")) return "Content too long. Try pasting fewer questions or a shorter section.";
  if (msg.includes("rate") || msg.includes("429") || msg.includes("busy")) return "AI is busy right now. Please wait a few seconds and try again.";
  if (msg.includes("401") || msg.includes("Invalid API")) return "Server configuration error. Please contact support.";
  return "Something went wrong. Please try again.";
}

async function callGroq(key: string, model: string, messages: any[], maxTokens = 3000): Promise<string> {
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (res.status === 429 || res.status === 503) {
      if (attempt < 3) { await new Promise(r => setTimeout(r, attempt * 4000)); continue; }
...
async function callText(messages: any[], maxTokens = 3000, isPremium = false): Promise<string> {
  const keys = [
  ...
  for (const key of unique) {
    try { return await callGroq(key, TEXT_MODEL, messages, maxTokens); }
      ...
      if (e.message.includes("rate") || e.message.includes("429") || e.message.includes("busy") || e.message.includes("TPD")) continue;
      throw e;
    }
  }
...
EXACTLY ${count} questions. Answer = A/B/C/D. Always include explanation.`;
  if (type === "pq_quiz") return `Convert past exam questions into a quiz.
Return ONLY a JSON array, no markdown. ${math} ${opts}
Format: ${fmt}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!prompt) return NextResponse.json({ error: "Invalid request type." }, { status: 400 });

    // ── VISION PATH (scanned PDF — beta, max 5 pages) ─────────────────────────
    if (images && images.length > 0) {
      const pages = images.slice(0, MAX_VISION_PAGES);
      console.log("[generate] vision: processing", pages.length, "pages (max", MAX_VISION_PAGES, ")");

      // Step 1: Extract text — Gemini first, Groq vision fallback
      let extracted = "";
      try {
        extracted = await callGemini(pages, VISION_EXTRACT_PROMPT);
        console.log("[generate] Gemini extracted:", extracted.length, "chars");
      } catch (e: any) {
        ...
        try {
          ...
          console.log("[generate] Groq vision extracted:", extracted.length, "chars");
        } catch (e2: any) {
          console.error("[generate] both vision models failed:", e2.message);
          return NextResponse.json({
            error: "Could not read the scanned PDF. Scanned PDF support is in beta — try copying the text and pasting it instead."
          }, { status: 422 });
        }
      }
      ...
      if (!extracted || extracted.length < 20) {
        return NextResponse.json({
          error: "Could not extract text from the PDF. Try copying the text and pasting it instead."
        }, { status: 422 });
      }

      // Step 2: Convert extracted text to questions
      const safe = truncate(fixLatex(extracted), MAX_CHARS);
