  ...
  try {
    ...
    if (recipients === "specific" && specificEmail) {
      ...
      snap.docs.forEach(d => {
        ...
        if (!d.id.startsWith("guest_") && data.email) {
          if (recipients === "all" || (recipients === "premium" && data.isPremium) || (recipients === "free" && !data.isPremium)) {
    ...
    const res = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        subject,
        htmlContent: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:20px"><div style="background:linear-gradient(135deg,#1e3a8a,#0891b2);padding:20px;border-radius:12px 12px 0 0;text-align:center"><h1 style="color:white;margin:0">Studiengine</h1></div><div style="background:#f8fafc;padding:24px;border-radius:0 0 12px 12px">${body.replace(/\n/g, "<br>")}<hr style="margin:24px 0;border:none;border-top:1px solid #e2e8f0"><p style="color:#94a3b8;font-size:12px;text-align:center">AI-powered CBT exam prep for Nigerian students</p></div></div>`,
      }),
    });
    if (!res.ok) {
