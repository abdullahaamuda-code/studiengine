...
export default function InputPanel({ onSubmit, loading, progress = 0, placeholder, buttonLabel, hint }: InputPanelProps) {
  ...
  async function handleFile(f: File) {
    ...
    setFile(f); setExtractError(""); setIsScanned(false); setVisionImages(null); setExtracting(true);
    try {
      const result = await extractTextFromPDF(f);
      if (isVisionPayload(result)) {
        const images = parseVisionPayload(result);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {extracting ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              ...
              <p style={{ color: "var(--text-muted)", fontSize: 12 }}>
                {formatFileSize(file.size)} • {isScanned ? `${visionImages?.length} pages detected` : "Text extracted"}
              </p>
              <button onClick={e => { e.stopPropagation(); setFile(null); setText(""); setVisionImages(null); setIsScanned(false); }}
      ...
      <button className="btn-primary" onClick={handleSubmit} disabled={!canSubmit}
        style={{ padding: "14px 0", borderRadius: 12, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, flexDirection: "column", overflow: "hidden", position: "relative" }}>
        {loading ? (
          <>
