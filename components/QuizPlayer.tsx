    ...
    try {
      const res = await fetch("/api/explain", {
        ...
        body: JSON.stringify({
          ...
          userQuestion:
            explainQ ||
            "Please explain this in detail with full working and reasoning.",
        }),
      });
      const data = await res.json();
      setExplainText(
        data.explanation || data.error || "Could not get explanation."
      );
    } catch {
      setExplainText("Network error. Please try again.");
    }
    setExplaining(false);
  }
    ...
    const grade =
      finalPct >= 80
        ? {
            ...
            label: "Excellent!",
            color: "#4ade80",
            msg: "You know this material. Keep it sharp.",
          }
        : finalPct >= 60
        ? {
            emoji: "📚",
            label: "Good effort",
            color: "#fbbf24",
            msg: "Review the ones you got wrong before exam day.",
          }
        : {
            emoji: "💪",
            label: "Keep grinding",
            color: "#f87171",
            msg: "More revision needed. Focus on the missed topics.",
          };
    ...
    return (
      ...
      >
        ...
        <div style={{ textAlign: "center", padding: "24px 20px" }}>
          ...
          {isPremium && saved && (
            <p style={{ fontSize: 11, color: "#4ade80", marginTop: 10 }}>
              ✓ Saved to history
            </p>
          )}
        </div>
        ...
        >
          ...
          {!isPremium && (
            ...
            >
              ...
              >
                ⚡ Upgrade Premium for full review + AI explanations
              </p>
            </div>
          )}
          {wrongOnes.length > 0 && (
                ...
                >
                  <span
                    style={{
                      fontSize: 11,
                      fontWeight: 700,
                      color: wrongOnes.some((w) => w.id === rq.id)
                        ? "#f87171"
                        : "#4ade80",
                      flexShrink: 0,
                      marginTop: 2,
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ borderRadius: 16, padding: "20px 18px", marginBottom: 14 }}
      >
        {q.year && (
          <span
            style={{
      ...
      {revealed && q.explanation && (
        <div
          className="animate-fade glass-static"
          style={{
            borderRadius: 12,
            padding: "12px 14px",
            marginBottom: 14,
