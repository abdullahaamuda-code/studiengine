...
export default function QuizPlayer({
  ...
  useEffect(() => {
    function handleKey(e: KeyboardEvent) {
      if (done) return;
      if (!revealed) {
        const idx = ({ a: 0, b: 1, c: 2, d: 3 } as any)[e.key.toLowerCase()];
  ...
  async function analyzeTopics(wrongs: Question[], corrects: Question[]) {
    ...
    try {
      ...
      const wrongSummary = wrongs.map((q) => q.question.slice(0, 120)).join(" | ");
      const correctSummary = corrects.slice(0, 5).map((q) => q.question.slice(0, 80)).join(" | ");
...
Keep topic names short (2-4 words max). Max 4 items per array.`,
      ...
      setTopicAnalysis({
        struggled: wrongs.slice(0, 3).map((q) => q.question.split(" ").slice(0, 4).join(" ")),
        strong: corrects.slice(0, 2).map((q) => q.question.split(" ").slice(0, 4).join(" ")),
        advice: "Revisit the topics where you made mistakes and re-read those sections.",
      });
    }
  ...
  if (done) {
    ...
    const grade =
      finalPct >= 80
        ? { emoji: "🔥", label: "Excellent work", color: "#34d399", sub: "You clearly know this material. Keep the momentum going." }
        : finalPct >= 60
          ? { emoji: "📚", label: "Good effort", color: "#fbbf24", sub: "You're on the right track. A bit more revision will get you there." }
          : { emoji: "💪", label: "Keep going", color: "#f87171", sub: "This is how you find the gaps. Now you know what to fix." };
      ...
      <>
        ...
        <div className="res-wrap">
          <div style={{ textAlign: "center", padding: "24px 16px 16px" }}>
            ...
            <div style={{ position: "relative", width: 100, height: 100, margin: "0 auto 14px" }}>
              <svg width="100" height="100" style={{ transform: "rotate(-90deg)" }}>
                ...
                <circle
                  ...
                  style={{ transition: "stroke-dashoffset 1.2s cubic-bezier(0.4,0,0.2,1)", filter: `drop-shadow(0 0 6px ${grade.color}88)` }}
                />
              </svg>
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
                <span style={{ fontSize: 22, fontWeight: 800, color: grade.color, fontFamily: "var(--font-display)", letterSpacing: "-0.03em", lineHeight: 1 }}>{finalPct}%</span>
            ...
            <p style={{ fontSize: 13, color: "#475569", margin: 0, lineHeight: 1.65, maxWidth: 280, marginLeft: "auto", marginRight: "auto" }}>
              {grade.sub}
            </p>
            {isPremium && saved && <p style={{ fontSize: 11, color: "#34d399", marginTop: 10, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>✓ Saved to history</p>}
          </div>
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "14px 16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              <div style={{ width: 28, height: 28, borderRadius: 8, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13 }}>◈</div>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#f1f5f9", margin: 0 }}>Performance breakdown</p>
            </div>

            {analyzingTopics ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0" }}>
                {[0, 1, 2].map((i) => (
                  <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#818cf8", animation: `dot-pulse 1.2s ease-in-out ${i * 0.2}s infinite` }} />
                ))}
                <span style={{ fontSize: 12, color: "#475569" }}>Analysing your results…</span>
              </div>
            ) : topicAnalysis ? (
              <div>
                {topicAnalysis.struggled.length > 0 && (
                  <div style={{ marginBottom: 12 }}>
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#f87171", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 8 }}>Needs work</p>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {topicAnalysis.struggled.map((t, i) => (
                        <span key={t} className="topic-pill-bad" style={{ animationDelay: `${i * 0.08}s` }}>
                ...
                {topicAnalysis.strong.length > 0 && (
                  <div style={{ marginBottom: 12 }}>
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#34d399", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 8 }}>You did well in</p>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {topicAnalysis.strong.map((t, i) => (
                        <span key={t} className="topic-pill-good" style={{ animationDelay: `${i * 0.08}s` }}>
                          <span style={{ fontSize: 9 }}>✓</span>
                          {t}
                        </span>
                      ))}
                ...
                {topicAnalysis.advice && (
                  <div style={{ padding: "10px 12px", background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.15)", borderRadius: 10, fontSize: 12.5, color: "#94a3b8", lineHeight: 1.65, animation: "advice-in 0.5s ease 0.3s both" }}>
                    <span style={{ color: "#818cf8", fontWeight: 700 }}>Advice: </span>
                    {topicAnalysis.advice}
                  </div>
                )}

                {wrongOnes.length > 0 && (
                  <button className="topics-toggle" onClick={() => setShowTopics((t) => !t)} style={{ marginTop: 10 }}>
                    <span>Questions you missed ({wrongOnes.length})</span>
                    <span style={{ fontSize: 12, transform: showTopics ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>▾</span>
                  </button>
                )}
                ...
                {showTopics && wrongOnes.length > 0 && (
                  <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 0 }}>
                    {wrongOnes.map((wq, i) => (
                      <div key={i} style={{ padding: "9px 0", borderBottom: i < wrongOnes.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none", display: "flex", gap: 8, alignItems: "flex-start" }}>
                        <span style={{ color: "#f87171", fontSize: 11, flexShrink: 0, marginTop: 2, fontWeight: 700 }}>✗</span>
                        <div style={{ flex: 1 }}>
                          <p style={{ fontSize: 12.5, color: "#f1f5f9", margin: "0 0 3px", lineHeight: 1.5 }}>
                            <MathText text={wq.question.slice(0, 100) + (wq.question.length > 100 ? "…" : "")} />
                          </p>
                          <p style={{ fontSize: 11.5, color: "#34d399", margin: 0 }}>
                            ✓ {wq.answer}.{" "}
                            <MathText text={wq.options?.find((o) => o.charAt(0).toUpperCase() === wq.answer?.toUpperCase())?.slice(3) || ""} />
                          </p>
                        </div>
                      </div>
                    ))}
              ...
              <p style={{ fontSize: 12.5, color: "#334155", margin: 0 }}>No analysis available.</p>
            )}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {isPremium ? (
              <button
                ...
                onClick={() => reviewSessionId && router.push(`/review/${reviewSessionId}`)}
                style={{ background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.28)", color: "#fbbf24", boxShadow: "0 0 0 0 rgba(251,191,36,0)", opacity: savingReview ? 0.6 : 1 }}
              >
            ...
            {shareUrl && (
              <div style={{ padding: "8px 12px", background: "rgba(56,189,248,0.05)", border: "1px solid rgba(56,189,248,0.14)", borderRadius: 9, fontSize: 11.5, color: "#38bdf8", wordBreak: "break-all", textAlign: "center" }}>
                {shareUrl}
              </div>
            )}
            ...
            <button
              ...
              onClick={onReset}
              style={{ background: "linear-gradient(135deg,#6366f1,#4f46e5,#4338ca)", border: "1px solid rgba(129,140,248,0.4)", color: "#fff", boxShadow: "0 4px 20px rgba(99,102,241,0.3)" }}
            >
              ← New CBT
            </button>
          </div>
  ...
  return (
    <>
      ...
      <div>
        {notice && (
          <div style={{ marginBottom: 12, display: "flex", gap: 8, padding: "9px 12px", background: "rgba(251,191,36,0.06)", border: "1px solid rgba(251,191,36,0.18)", borderRadius: 10, fontSize: 12.5, color: "#fde68a" }}>
            <span>ℹ️</span>
            {notice}
          </div>
        )}
        ...
        {subject && (
          <div style={{ marginBottom: 10 }}>
            <span style={{ fontSize: 11, color: "#818cf8", background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", padding: "3px 10px", borderRadius: 99, fontWeight: 600 }}>
              📚 {subject}
            </span>
          </div>
        )}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <span style={{ fontSize: 12.5, color: "var(--text-muted)", fontFamily: "var(--font-display)", fontWeight: 600 }}>
            {current + 1} <span style={{ color: "#334155" }}>/ {total}</span>
          </span>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 12, color: "#34d399", fontWeight: 700, background: "rgba(52,211,153,0.08)", border: "1px solid rgba(52,211,153,0.2)", padding: "3px 9px", borderRadius: 99 }}>
              ✓ {score}
            </span>
            {timeLeft !== null && (
              <span style={{ fontSize: 12, fontWeight: 800, color: timeLeft < 60 ? "#f87171" : "#fbbf24", background: timeLeft < 60 ? "rgba(239,68,68,0.1)" : "rgba(251,191,36,0.1)", border: `1px solid ${timeLeft < 60 ? "rgba(239,68,68,0.3)" : "rgba(251,191,36,0.25)"}`, padding: "3px 9px", borderRadius: 99, fontFamily: "var(--font-display)" }}>
                ⏱ {formatTime(timeLeft)}
              </span>
            )}
            <button className="quit-btn" onClick={onReset} style={{ fontSize: 11.5, padding: "4px 11px", borderRadius: 7, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", color: "var(--text-muted)", cursor: "pointer", fontFamily: "var(--font-body)", transition: "all 0.15s" }}>
              Quit
            </button>
          </div>
        </div>

        <div style={{ height: 3, background: "rgba(255,255,255,0.05)", borderRadius: 99, marginBottom: 4, overflow: "hidden" }}>
          <div style={{ height: "100%", width: `${pct}%`, background: "linear-gradient(90deg,#6366f1,#38bdf8)", borderRadius: 99, transition: "width 0.4s ease", boxShadow: "0 0 8px rgba(99,102,241,0.4)" }} />
        </div>
        <p style={{ fontSize: 10, color: "#1e293b", textAlign: "right", marginBottom: 18 }}>A B C D · Space/Enter for next</p>

        <div key={animKey} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "20px 18px", marginBottom: 12, animation: "qp-in 0.3s cubic-bezier(0.4,0,0.2,1)" }}>
          {q.year && <span style={{ fontSize: 10.5, color: "#818cf8", background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", padding: "3px 10px", borderRadius: 99, display: "inline-block", marginBottom: 12, fontWeight: 600 }}>{q.year}</span>}
          <p style={{ fontSize: 15, color: "#f1f5f9", lineHeight: 1.75, fontWeight: 500, margin: 0 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 14 }}>
          {q.options.map((opt) => {
            ...
            return (
              <button key={opt} onClick={() => pick(opt)} className={cls} disabled={revealed}>
                <span style={{ width: 24, height: 24, borderRadius: 7, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, background: revealed && isCorrect ? "rgba(52,211,153,0.25)" : revealed && isSelected ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.06)", color: revealed && isCorrect ? "#34d399" : revealed && isSelected ? "#f87171" : "#475569" }}>
        ...
        {revealed && q.explanation && (
          <div style={{ borderRadius: 12, padding: "13px 15px", marginBottom: 12, background: "rgba(99,102,241,0.07)", border: "1px solid rgba(99,102,241,0.18)", animation: "qp-fade 0.3s ease" }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#818cf8", marginBottom: 5, letterSpacing: "0.06em", textTransform: "uppercase" }}>Explanation</p>
